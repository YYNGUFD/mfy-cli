 ...
 * add 
 */
const { fse, inquirer ,chalk} = require("../tools/module");
...
module.exports = async function (fileDir, options) {
  ...
  if(path.isAbsolute(fileDir)){
    log(chalk.red.bold("pleacse input relative path"))
    process.exit(0)
  }
  ...
  if(fse.pathExistsSync(fileDirCurrent)){
    log(chalk.red.bold("the file have existed")); 
  ...
  if(templateConfig.indexOf(resultExt)!=-1){ 
   await fse.copy(tplPath, fileDirCurrent, err => {
      ...
      log(chalk.green.bold("File creation successful"))
    })
  }else{
    //3.进行创建文件 先获取创建文件的路径  
    await fse.ensureFile(fileDirCurrent,err=>{
      if(err){
        ...
        log(chalk.green.bold("File created successful"))
      } 
    }) 
  }
}